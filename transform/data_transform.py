...
class DataTransformer(object):
    ...
    def convert_hours_to_minute(self, column_to_convert_minutes: str = 'average_answer_time_in_hours'):
        ...
        new_columnname = column_to_convert_minutes[:-5] + 'min'
...
# compute extra time
    @classmethod
    def calculate_extra_time_min(self, data_to_transform: pd.DataFrame, # = None, 
                                 max_time_min="max_task_time_in_min", 
    ...
    @classmethod
    def transform_target_variable(cls, data: pd.DataFrame,
                                  target_variable: str = args.target_variable,                                  
                                  bin_values: str = args.not_passed,
                                  transform_column_name: str = 'state_category'
                                  ):
        if bin_values:
            transform_state_variable = lambda x: 0 if x in bin_values else 1
            data[transform_column_name] = data[target_variable].apply(transform_state_variable)
            return data
